...
def linear_regression():
    ...
    Theta = reg.ls_solve(trainXones, trainY)[0] #since ls_solve also returns error
    #---------------------------------------------------------------------#

    fig1 = plt.figure(figsize=(10,10))
    ax1 = fig1.add_subplot(111)
    util.plot_regression(trainXones, trainY, Theta, ax1)
    ax1.grid()
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    ...
    #---------------------------------------------------------------------#
    # calculating error using root mean square error
    trainY_pred = trainXones.dot(Theta)
    testXones = util.addones(testX)
    testY_pred = testXones.dot(Theta)

    E_validation = np.sqrt(np.mean((trainY - trainY_pred)**2))
    E_test = np.sqrt(np.mean((testY - testY_pred)**2))
...
def quadratic_regression():
    ...
    trainX = train_data[:,0].reshape(-1,1)
    trainXquad = np.column_stack((trainX, trainX**2))
    trainXones = util.addones(trainXquad)
    trainY = train_data[:,1].reshape(-1,1)

    weights, error = reg.ls_solve(trainXquad, trainY) #(3,1)
 
   

    fig1 = plt.figure(figsize=(10,10))
    ax1 = fig1.add_subplot(111)
    util.plot_regression(trainXquad, trainY, Theta, ax1)
    ax1.grid()
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    ax1.legend(('Original data', 'Regression curve', 'Predicted Data', 'Error'))
    ...
    testX = test_data[:,0].reshape(-1,1)
    #testXones = util.addones(trainX)
    testXquad = np.column_stack((testX, testX**2))
    testY = test_data[:,1].reshape(-1,1)

    fig2 = plt.figure(figsize=(10,10))
    ax2 = fig2.add_subplot(111)
    util.plot_regression(testXquad, testY, Theta, ax2)
    ax2.grid()
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.legend(('Original data', 'Regression curve', 'Predicted Data', 'Error'))
    ...
    # calculating error using root mean square error
    trainY_pred = trainXones.dot(Theta)
    testXones = util.addones(testXquad)
    testY_pred = testXones.dot(Theta)
...
def logistic_regression():
    ...
    Theta = 0.02*np.random.rand(c+1, 1) #vector 3x1
...
def covariance_matrix_test():
    ...
    # Calculate mean and covariance matrix for X 
    mean_X = np.mean(X, axis=0)
    cov_X = np.cov(X, rowvar=False)

    print("Calculated mean of X: ", mean_X)
    print("Input mean of X (mu1): ", mu1)
    print("Calculated covariance of X: \n", cov_X)
    print("Input covariance of X (sigma1): \n", sigma1)
...
def eigen_vecval_test(sigma):
    ...
    for i in range(len(v)):
        ...
        normalization_results.append((i+1, norm))
        
    #orthogonality  -> dot product close to 0 
    for i in range(len(v)):
